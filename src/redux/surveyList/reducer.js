...
export default (state = INIT_STATE, action) => {
    switch (action.type) {
        ...
        case SURVEY_LIST_GET_LIST_SUCCESS:
            return {
                ...state, loading: true, allSurveyItems: action.payload, surveyItems: action.payload,
            };
        ...
        case SURVEY_LIST_GET_LIST_WITH_FILTER:
            if (action.payload.column === '' || action.payload.value === '') {
                return {
                    ...state, loading: true, surveyItems: state.allSurveyItems, filter: null,
                };
            }
            const filteredItems = state.allSurveyItems.filter(item => item[action.payload.column] === action.payload.value);
            return {
                ...state,
                loading: true,
        ...
        case SURVEY_LIST_GET_LIST_WITH_ORDER:
            if (action.payload === '') {
                return {
                    ...state, loading: true, surveyItems: state.surveyItems, orderColumn: null,
                };
            }
            ...
            return {
                ...state, loading: true, surveyItems: sortedItems, orderColumn: state.orderColumns.find(x => x.column === action.payload),
            };
        ...
        case SURVEY_LIST_GET_LIST_SEARCH:
            ...
            const searchItems = state.allSurveyItems.filter(item => item.title.toLowerCase().indexOf(keyword) > -1 || item.detail.toLowerCase().indexOf(keyword) > -1 || item.status.toLowerCase().indexOf(keyword) > -1 || item.category.toLowerCase().indexOf(keyword) > -1 || item.label.toLowerCase().indexOf(keyword) > -1);
            return {
                ...state, loading: true, surveyItems: searchItems, searchKeyword: action.payload,
            };
