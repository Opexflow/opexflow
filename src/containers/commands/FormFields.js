...
import { Col,  FormGroup, Label, Input, CustomInput, InputGroup, InputGroupAddon, InputGroupText, FormFeedback } from 'reactstrap';
            ...
            <Input 
              ...
              onChange={props.handleInputChange}>
              {
                commands.map((command, index) => {
                  return command.fields && command.fields.cond_type ? 
                  command.fields.cond_type.map((condType, index) =>
                  <option key={`${index}`} value={condType}>{condType}</option>) : null
              })}
            </Input>
          </FormGroup>
        </Col>
